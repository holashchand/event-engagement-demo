...
/**
 * Test for /api/v1/Visitor/{entityId}
 */
Test('/api/v1/Visitor/{entityId}', function (t) {
        ...
        t.test('test  get operation', function (t) {
            Mockgen().requests({
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/api/v1/Visitor/{entityId}']['get']['responses']['200']['schema']);
        ...
        t.test('test  put operation', function (t) {
            Mockgen().requests({
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/api/v1/Visitor/{entityId}']['put']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
