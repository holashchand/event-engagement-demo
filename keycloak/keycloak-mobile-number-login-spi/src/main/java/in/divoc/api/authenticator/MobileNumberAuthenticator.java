...
public class MobileNumberAuthenticator extends AbstractUsernameFormAuthenticator implements Authenticator {
	...
	public void action(AuthenticationFlowContext context) {
		...
		if (type.equals(LOGIN_FORM)) {
			...
			List<String> mobileNumberNodes = user.getAttributes().getOrDefault(MOBILE_NUMBER, Collections.emptyList());
			if (mobileNumberNodes.size() > 0) {
				mobileNumber = mobileNumberNodes.get(0);
			}
			if (mobileNumber != null) {
				String secret = formData.getFirst(OTP);
				String VALID_OTP = "VALID_OTP";
				String VERIFICATION_SERVICE = "VERIFICATION_SERVICE";
				if (secret != null) {
					try {
						if (notifyService.validateOTP(mobileNumber, secret)) {
							context.success();
						} else {
						...
						Response response = context.form().setError(System.getenv().getOrDefault(VERIFICATION_SERVICE, "OTP Check failed, please try after sometime")).createForm(VERIFY_OTP_UI);
						if(checkIfMaxOtpTriesReached(context)) {
							return;
						}
